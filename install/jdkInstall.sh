...
do
    ...
    ##增加xcall环境变量
    ssh root@${insName}  "echo '#XCALL_HOME'>>/etc/profile ;echo export XCALL_HOME=$XCALL_HOME >> /etc/profile"
    ssh root@${insName} 'echo export PATH=\$XCALL_HOME/tool:\$PATH  >> /etc/profile; echo "">> /etc/profile' 
